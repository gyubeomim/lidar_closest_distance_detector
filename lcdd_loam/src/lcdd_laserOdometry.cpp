...
PointCloud::Ptr imuTrans(new PointCloud());
pcl::KdTreeFLANN<VPoint>::Ptr kdtreeCornerLast(new pcl::KdTreeFLANN<VPoint>());
pcl::KdTreeFLANN<VPoint>::Ptr kdtreeSurfLast(new pcl::KdTreeFLANN<VPoint>());
...
void TransformToStart(VPoint *pi, VPoint *po) {
...
void TransformToEnd(VPoint *pi, VPoint *po) {
  float s = 10 * (pi->intensity - int(pi->intensity));
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "ps_laserOdometry");
  ros::NodeHandle nh;
  ...
  VPoint pointOri, pointSel, tripod1, tripod2, tripod3, pointProj, coeff;
  ...
  while (status) {
    ...
    if (newCornerPointsSharp && newCornerPointsLessSharp && newSurfPointsFlat &&
      ...
      if (!systemInited) {
        VPointCloud::Ptr laserCloudTemp = cornerPointsLessSharp;
