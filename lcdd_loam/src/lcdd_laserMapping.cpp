...
VPointCloud::Ptr laserCloudCornerLast(new VPointCloud());
VPointCloud::Ptr laserCloudSurfLast(new VPointCloud());
VPointCloud::Ptr laserCloudCornerStack(new VPointCloud());
VPointCloud::Ptr laserCloudSurfStack(new VPointCloud());
VPointCloud::Ptr laserCloudCornerStack2(new VPointCloud());
VPointCloud::Ptr laserCloudSurfStack2(new VPointCloud());
VPointCloud::Ptr laserCloudOri(new VPointCloud());
VPointCloud::Ptr coeffSel(new VPointCloud());
VPointCloud::Ptr laserCloudSurround(new VPointCloud());
VPointCloud::Ptr laserCloudSurround2(new VPointCloud());
VPointCloud::Ptr laserCloudCornerFromMap(new VPointCloud());
VPointCloud::Ptr laserCloudSurfFromMap(new VPointCloud());
VPointCloud::Ptr laserCloudFullRes(new VPointCloud());
VPointCloud::Ptr laserCloudCornerArray[laserCloudNum];
VPointCloud::Ptr laserCloudSurfArray[laserCloudNum];
VPointCloud::Ptr laserCloudCornerArray2[laserCloudNum];
VPointCloud::Ptr laserCloudSurfArray2[laserCloudNum];

pcl::KdTreeFLANN<VPoint>::Ptr kdtreeCornerFromMap(new pcl::KdTreeFLANN<VPoint>());
pcl::KdTreeFLANN<VPoint>::Ptr kdtreeSurfFromMap(new pcl::KdTreeFLANN<VPoint>());
...
void pointAssociateToMap(VPoint *pi, VPoint *po)
{
...
void pointAssociateTobeMapped(VPoint *pi, VPoint *po)
{
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "ps_laserMapping");
  ros::NodeHandle nh;
  ...
  VPoint pointOri, pointSel, pointProj, coeff;
  ...
  pcl::VoxelGrid<VPoint> downSizeFilterCorner;
  downSizeFilterCorner.setLeafSize(0.4, 0.4, 0.4);

  pcl::VoxelGrid<VPoint> downSizeFilterSurf;
  downSizeFilterSurf.setLeafSize(0.6, 0.6, 0.6);

  pcl::VoxelGrid<VPoint> downSizeFilterMap;
  downSizeFilterMap.setLeafSize(0.6, 0.6, 0.6);

  for (int i = 0; i < laserCloudNum; i++) {
    laserCloudCornerArray[i].reset(new VPointCloud());
    laserCloudSurfArray[i].reset(new VPointCloud());
    laserCloudCornerArray2[i].reset(new VPointCloud());
    laserCloudSurfArray2[i].reset(new VPointCloud());
  }
    ...
    if (newLaserCloudCornerLast && newLaserCloudSurfLast && newLaserCloudFullRes && newLaserOdometry &&
      ...
      if (frameCount >= stackFrameNum) {
        ...
        while (centerCubeI < 3) {
          for (int j = 0; j < laserCloudHeight; j++) {
            for (int k = 0; k < laserCloudDepth; k++) {
              ...
              VPointCloud::Ptr laserCloudCubeCornerPointer =
              laserCloudCornerArray[i + laserCloudWidth * j + laserCloudWidth * laserCloudHeight * k];
              VPointCloud::Ptr laserCloudCubeSurfPointer =
        ...
        for (int i = 0; i < laserCloudValidNum; i++) {
          ...
          VPointCloud::Ptr laserCloudTemp = laserCloudCornerArray[ind];
