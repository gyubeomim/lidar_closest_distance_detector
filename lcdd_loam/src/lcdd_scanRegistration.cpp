...
bool loam_toggle;
std_msgs::Bool loam_toggle_msg;
ros::Publisher pub_loam_toggle;
...
PointCloud::Ptr laserCloudIn(new PointCloud());
VPointCloud::Ptr laserCloud(new VPointCloud());
VPointCloud::Ptr cornerPointsSharp(new VPointCloud());
VPointCloud::Ptr cornerPointsLessSharp(new VPointCloud());
VPointCloud::Ptr surfPointsFlat(new VPointCloud());
VPointCloud::Ptr surfPointsLessFlat(new VPointCloud());
VPointCloud::Ptr surfPointsLessFlatScan(new VPointCloud());
VPointCloud::Ptr surfPointsLessFlatScanDS(new VPointCloud());
PointCloud::Ptr imuTrans(new PointCloud(4, 1));
VPointCloud::Ptr laserCloudScans[16];
...
void TransformToStartIMU(VPoint *p)
{
...
void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudIn2) {
  loam_toggle_msg.data = loam_toggle;
  pub_loam_toggle.publish(loam_toggle);
  ...
  for (int i = 0; i < cloudSize_tmp; i++) {
    ...
    // COMMENT(edward): We only use 16 channels of Velodyne-HDL64S2 Sensor due to the reduction of computational operation
  ...
  for (int i = 0; i < 16; i++) {
    ...
    pcl::VoxelGrid<VPoint> downSizeFilter;
...
void loam_toggle_callback(const geometry_msgs::PointStamped::ConstPtr &msg) {
  if(loam_toggle==true)
    loam_toggle = false;
  else
    loam_toggle = true;
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "ps_scanRegistration");
  ros::NodeHandle nh;

  for (int i = 0; i < 16; i++) {
    laserCloudScans[i].reset(new VPointCloud());
  }

  ros::Subscriber subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2>("/velodyne_points", 2, laserCloudHandler);
  ros::Subscriber sub_loam_toggle = nh.subscribe<geometry_msgs::PointStamped>("/clicked_point", 5, loam_toggle_callback);
  ...
  pub_loam_toggle = nh.advertise<std_msgs::Bool>("loam_toggle", 1);
