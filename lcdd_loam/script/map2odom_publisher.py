...
class Map2OdomPublisher:
	def __init__(self):
		...
		self.subscriber = rospy.Subscriber('/ps_graph_slam/odom2pub', TransformStamped, self.callback)
	...
	def spin(self):
		if not hasattr(self, 'odom_msg'):
			self.broadcaster.sendTransform((0, 0, 0), (0, 0, 0, 1), rospy.Time.now(), 'odom', 'camera_init')
			return
